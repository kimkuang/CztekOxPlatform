...
TcpServer::TcpServer(uint port, QObject *eventReceiver)
{
    m_port = port;
...
int TcpServer::StartListen(const QString &address)
{
//    qDebug()<<__FUNCTION__;
    QHostAddress hostAddress = QHostAddress(address);
    if (hostAddress.isNull())
        hostAddress = QHostAddress::LocalHost;
    if (!m_tcpServer->listen(QHostAddress::Any, m_port)/*m_tcpServer->listen(hostAddress, m_port)*/) {
        m_tcpServer->close();
        qCritical()<<QString("Listening on %1@%2 failed.").arg(hostAddress.toString()).arg(m_port);
        return ERR_Failed;
    }
    qDebug()<<QString("Listening on %1@%2 ...").arg(hostAddress.toString()).arg(m_port);
    return ERR_NoError;
}
...
//    QString strBind1, strBind2;
    while (((tcpSocket->bytesAvailable() > 0) || (strPduCache.size() > 0)) && (ec == ERR_NoError)) {
        ...
        qInfo("port: %d, pdu cache: %s", m_port, strPduCache.c_str());
        ...
        if (ControlPdu::Receiver_TestItem == reqCtrlPdu.Receiver)
        {
            ...
            QCoreApplication::postEvent(m_eventReceiver, new MachineAntiShakeEvent(id, state));
        }
        else if (ControlPdu::Receiver_Tester == reqCtrlPdu.Receiver) {
            QCoreApplication::postEvent(m_eventReceiver, new MachineRequestEvent(reqCtrlPdu.Command,
                reqCtrlPdu.SubStation, reqCtrlPdu.CameraIndex, QString::fromStdString(reqCtrlPdu.Parameter)));
