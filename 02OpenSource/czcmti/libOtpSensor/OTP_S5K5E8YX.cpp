...
#define OTP_DRV_S5K5E8YX

#include "OTP_S5K5E8YX.h"

OTP_S5K5E8YX::OTP_S5K5E8YX(uint i2cAddr)
            : OtpSensor(i2cAddr)
{

}

OTP_S5K5E8YX::~OTP_S5K5E8YX(void)
{

}

int OTP_S5K5E8YX::OtpRead(uint startAddr, uint endAddr, uchar *Buff, ushort page)
{
    QString strLog = "";
    if (NULL == Buff)
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s Error---Buff:0x%p is NULL\r\n", __FUNCTION__, Buff);
    ...
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s Error---startAddr:%d is Larger than endAddr:%d\r\n", __FUNCTION__, startAddr, endAddr);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cRead Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    if (SENSOR_SOFTWARE_STANDBY == (stram_flag & SENSOR_OP_MODE_MASK))
    {
        ...
        {
            strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_OP_MODE_ADDR Error\r\n", __FUNCTION__);
            m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
            return ERR_Failed;
        }
        /**
    ...
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_CTL_RW_ADDR[PENDING_ERROR] Error---page:%d\r\n", __FUNCTION__, page);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    if (ERR_NoError != I2cWrite(REG_PAGE_SEL_ADDR, page, RB_ADDR16_DATA8))    //Set Page Num
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_PAGE_SEL_ADDR Error---page:%d\r\n", __FUNCTION__, page);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    while (1)
    {
        ...
        {
            strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cRead REG_ERR_FLGA_ADDR Error---iCnt:%d\r\n", __FUNCTION__, iCnt);
            m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
            return ERR_Failed;
        }
        ...
        if (iCnt >= 3)
        {
            strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cRead REG_ERR_FLGA_ADDR Error---iCnt:%d so Timeout\r\n", __FUNCTION__, iCnt);
            m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
            return ERR_Failed;
        }
    }
    ...
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---ReadContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    if (ERR_NoError != I2cWrite(REG_CTL_RW_ADDR, PENDING_ERROR, RB_ADDR16_DATA8))  //Make initial State
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_CTL_RW_ADDR Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OTP_S5K5E8YX::OtpWrite(uint startAddr, uint endAddr, const uchar *buf, ushort page)
{
    QString strLog = "";

    if (NULL == buf || endAddr < startAddr)
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s Error---buf:0x%p is NULL or startAddr:%d > endAddr:%d\r\n", __FUNCTION__, buf, startAddr, endAddr);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_OTP_WR_DISABLE_ADDR[OTP_WR_DISABLE_OFF] Error---page:%d\r\n", __FUNCTION__, page);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    if (ERR_NoError != I2cWrite(REG_OTP_WR_TIME_MODE_ADDR, WR_TIME_UNLIMITED_MODE, RB_ADDR16_DATA8))    //Set Page Num
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---I2cWrite REG_OTP_WR_TIME_MODE_ADDR[WR_TIME_UNLIMITED_MODE] Error---page:%d\r\n", __FUNCTION__, page);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    if (ERR_NoError != I2cWrite(REG_CTL_RW_ADDR, PENDING_ERROR, RB_ADDR16_DATA8)) //Make initial State
    ...
    int nLen = endAddr - startAddr + 1;
    if (ERR_NoError != m_channelController->WriteContinuousI2c(m_i2cAddr, 400, startAddr, 2, buf, /*PAGE_LEN*/nLen))
    {
        strLog = QString::asprintf("OTP_S5K5E8YX::%s---WriteContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
