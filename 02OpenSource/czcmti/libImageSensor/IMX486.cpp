...
// 参考文档《IMX486_Software_Reference_Manual_0.1.0.pdf》
// 参考OTP读写操作的文档《IMX486_OTP_Manual_v0.1.0.pdf》
...
int IMX486::GetTemperature(int &temperature)
{
    ...
    if ((tempCtlData & 0x01) != 0x01) {
        ...
        if (ec < 0) {
            QString strLog = "Wtrie IMX486 temperature control Register failed";
            m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
            qCritical() << strLog;
            return ec;
        }
    ...
    if (ec < 0) {
        QString strLog = "Read IMX486 temperature data Register failed";
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX486::GetSensorFuseId(std::string &fuseId, bool bUseMasterI2C)
{
	(void)bUseMasterI2C;
    uchar tmpBuff[FUSE_ID_LEN] = {0};
    int ec = readOtpData(FUSE_ID_ADDR_START, FUSE_ID_ADDR_END, tmpBuff, FUSE_ID_PAGE);
    if (ec < 0) {
        QString strLog = QString("Read IMX486 FuseId failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX486::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
    QString strLog = "";

    if (rg == 0 || bg == 0 || Typical_rg == 0 || Typical_bg == 0)
    {
        strLog = QString::asprintf("IMX486::%s Error---rg or bg or Typical_rg or Typical_bg is zero\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    if ( !r_ratio || !b_ratio)
    {
        strLog = QString::asprintf("IMX486::%s Error---r_ratio or b_ratio is zero\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int IMX486::GetSensorExposure(uint &value)
{
    uchar buf[] = {0, 0};
    int ec = i2cReadContinuous(EXPOSURE_REG_H_ADDR, 2, buf, ARRAY_SIZE(buf));
    if (ec < 0) {
        QString strLog = QString("IMX486::Call ReadContinuousI2c() failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX486::SetSensorExposure(uint value)
{
    uchar buf[] = {MSB(value), LSB(value)};
    int ec = i2cWriteContinuous(EXPOSURE_REG_H_ADDR, 2, buf, ARRAY_SIZE(buf));
    if (ec < 0) {
        QString strLog = QString("IMX486::Call WriteContinuousI2c() failed[%2]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX486::GetSensorGain(uint &value)
{
    uchar buf[] = {0, 0};
...
int IMX486::SetSensorGain(uint value, bool bMultiple)
{
    if (true == bMultiple)
    {
        value = 512 - (512 / value);
    ...
    if (ec < 0) {
        QString strLog = QString("IMX486::Call WriteContinuousI2c() failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX486::readOtpData(ushort startAddr, ushort endAddr, uchar *buf, ushort page)
{
    if (nullptr == buf) {
        qCritical("IMX486::%s Error---buf is NULL", __FUNCTION__);
        return ERR_InvalidParameter;
    }

    uint bufferLen = endAddr - startAddr + 1;
    if (bufferLen > PAGE_LEN) {
        qCritical("IMX486::%s Error---nLen:%d is outside PAGE_LEN:%d", __FUNCTION__, bufferLen, PAGE_LEN);
        return ERR_InvalidParameter;
    }
    ...
    if (ec < 0) {
        qCritical("Turn on IMX486 otp read mode failed[%d]", ec);
        return ec;
    }
    ...
    if (checkFailed) {
        qCritical("Check IMX486 OTP status[0x%x] failed", flag);
        return ERR_Failed;
    }
    ...
    if (ec < 0) {
        qCritical("IMX486 read otp addr[0x%x] failed[%d]", startAddr, ec);
        return ec;
    }
