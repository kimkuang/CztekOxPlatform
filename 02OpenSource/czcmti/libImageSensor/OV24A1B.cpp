...
OV24A1B::OV24A1B(const T_SensorSetting &sensorSetting) :
    ImageSensor(sensorSetting)
{
    qDebug()<<__FUNCTION__;
}

int OV24A1B::GetTemperature(int &temperature)
{
    QString strLog = "";
    USHORT temp = 0x00;
    ...
    {
        {0x3017, 0xF0},//en tmp--和OV16B10相比，多了这个寄存器的设置
        {0x4D00, 0x05},//slope
        {0x4D01, 0x00},//slope
        {0x4D02, 0xB7},//offset
        {0x4D03, 0xCA},//offset
    ...
    {
        strLog = QString::asprintf("OV24A1B::%s---I2cWrite Error---333\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        strLog = QString::asprintf("OV24A1B::%s---I2cRead Error---444\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
     ...
     * ----20180404
     */
    strLog = QString::asprintf("OV24A1B::%s---Temperature RegVal:0x%X\r\n", __FUNCTION__, temp);
    m_channelController->LogToWindow(strLog, qRgb(0, 0, 255));
    temperature = temp;
    if (temp > 0xC0)
    {
        /*strLog = QString::asprintf("OV24A1B::%s---Attention, temp:0x%X is negative, so fixed 0°\r\n", __FUNCTION__, temp);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));*/
    ...
    if (ERR_NoError != I2cWrite(TEMPERATURE_CTL_REG_ADDR, temp & 0xFE, I2C_MODE_ADDR16_DATA8)) //Clear Bit[0]
    {
        strLog = QString::asprintf("OV24A1B::%s---I2cWrite Error---555\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV24A1B::GetSensorFuseId(std::string &fuseId, bool bUseMasterI2C)
{
    (void)bUseMasterI2C;
    QString strLog = "";
    USHORT tempVal[16] = {0};
    if (ERR_NoError != ReadReg(0x7000, 0x700F, tempVal))   //OV16B10的Sensor上有描述0x7000~0x700F是Production ID，但是OV24A1B上没有，需要询问OV原厂
    {
        strLog = QString::asprintf("OV24A1B::%s---ReadReg Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV24A1B::GetSensorExposure(uint &value)
{
    QString strLog = "";
#if 1
    uchar Buf[] = {0, 0};
    ...
    {
        strLog = QString::asprintf("OV24A1B::%s---ReadContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV24A1B::SetSensorExposure(uint value)
{
    QString strLog = "";
#if 1
    ...
    {
        strLog = QString::asprintf("OV24A1B::%s---WriteContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV24A1B::GetSensorGain(uint &value)
{
    QString strLog = "";
#if 1
    uchar Buf[] = {0, 0};
...
int OV24A1B::SetSensorGain(uint value, bool bMultiple)
{
    (void)bMultiple;
    QString strLog = "";
#if 1
...
int OV24A1B::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf, USHORT page)
{
    ...
    if (nullptr == buf)
    {
        strLog = QString::asprintf("OV24A1B::%s---Error---buf is null", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV24A1B::ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf, USHORT page)
{
    ...
    if (startAddr > endAddr)
    {
        strLog = QString::asprintf("OV24A1B::%s---Error---startAddr larger than endAddr---startAddr:0x%X endAddr:0x%X", __FUNCTION__, startAddr, endAddr);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        qDebug() << __FUNCTION__ << "  Error---new ptmpBuf Error---" << startAddr << " endAddr:" << endAddr << " nLen:" << nLen;
        strLog = QString::asprintf("OV24A1B::%s---Error---new ptmpBuf Error---startAddr:0x%X endAddr:0x%X nLen:%d", __FUNCTION__, startAddr, endAddr, nLen);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        qDebug() << __FUNCTION__ << "  WriteContinuousI2c Error";
        strLog = QString::asprintf("OV24A1B::%s---WriteContinuousI2c Error---i2cParam.Addr:0x%X i2cParam.Speed:%d", __FUNCTION__, i2cParam.Addr, i2cParam.Speed);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return ec;
    }*/
    ...
    if (ERR_NoError != iRet)
    {
        strLog = QString::asprintf("OV24A1B::%s---writeRegisters Error--iRet:%d\r\n", __FUNCTION__, iRet);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return iRet;
    }
    ...
    if (ERR_NoError != iRet)
    {
        ...
        strLog = QString::asprintf("OV24A1B::%s---222---WriteContinuousI2c Error", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return ERR_Failed;
    }*/
