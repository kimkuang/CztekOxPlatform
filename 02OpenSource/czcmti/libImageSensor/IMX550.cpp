...
// 参考文档《IMX550-AAMH5_Software_Reference_Manual_0.1.0》
// 参考OTP读写操作的文档《IMX550-AAMH5_OTP_Manual_0.1.0.pdf》
...
IMX550::IMX550(const T_SensorSetting &sensorSetting) :
    ImageSensor(sensorSetting)
{
}

int IMX550::GetTemperature(int &temperature)
{
    ushort tempCtlData = 0;
    int ec = I2cRead(TEMPATURE_CTRL_ADDR, tempCtlData, I2C_MODE_ADDR16_DATA8);
    if (ec < 0) {
        QString strLog = "Read IMX550 TEMPATURE CTRL Register failed";
    ...
    if ((tempCtlData & 0x01) != 0x01) {
        ...
        if (ec < 0) {
            QString strLog = "Wtrie IMX550 temperature control Register failed";
            m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
            qCritical() << strLog;
            return ec;
        }
    ...
    if (ec < 0) {
        QString strLog = "Read IMX550 temperature data Register failed";
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX550::GetSensorFuseId(std::string &fuseId, bool bUseMasterI2C)
{
    (void)bUseMasterI2C;
    uchar tmpBuff[FUSE_ID_LEN] = {0};
    int ec = readOtpData(FUSE_ID_ADDR_START, FUSE_ID_ADDR_END, tmpBuff, FUSE_ID_PAGE);
    if (ec < 0) {
        QString strLog = QString("Read IMX550 FuseId failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX550::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
int IMX550::SetSensorExposure(uint value)
{
    uchar buf[] = {MSB(value), LSB(value)};
    int ec = i2cWriteContinuous(EXPOSURE_REG_H_ADDR, 2, buf, ARRAY_SIZE(buf));
    if (ec < 0) {
        QString strLog = QString("IMX550::Call WriteContinuousI2c() failed[%2]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX550::SetSensorGain(uint value, bool bMultiple)
{
    if (true == bMultiple)
    {
    ...
    if (ec < 0) {
        QString strLog = QString("IMX550::Call WriteContinuousI2c() failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX550::GetSensorExposure(uint &value)
{
    uchar buf[] = {0, 0};
    int ec = i2cReadContinuous(EXPOSURE_REG_H_ADDR, 2, buf, ARRAY_SIZE(buf));
    if (ec < 0) {
        QString strLog = QString("IMX550::Call ReadContinuousI2c() failed[%1]").arg(ec);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        qCritical() << strLog;
        return ec;
    }
...
int IMX550::GetSensorGain(uint &value)
{
    uchar buf[] = {0, 0};
...
int IMX550::readOtpData(ushort startAddr, ushort endAddr, uchar *buf, ushort page)
{
    if (nullptr == buf) {
        qCritical("IMX550::%s Error---buf is NULL", __FUNCTION__);
        return ERR_InvalidParameter;
    }

    uint bufferLen = endAddr - startAddr + 1;
    if (bufferLen > PAGE_LEN) {
        qCritical("IMX550::%s Error---nLen:%d is outside PAGE_LEN:%d", __FUNCTION__, bufferLen, PAGE_LEN);
        return ERR_InvalidParameter;
    }
    ...
    if (ec < 0) {
        qCritical("Turn on IMX550 otp read mode failed[%d]", ec);
        return ec;
    }
    ...
    if (checkFailed) {
        qCritical("Check IMX550 OTP status[0x%x] failed", flag);
        return ERR_Failed;
    }
    ...
    if (ec < 0) {
        qCritical("IMX550 read otp addr[0x%x] failed[%d]", startAddr, ec);
        return ec;
    }
