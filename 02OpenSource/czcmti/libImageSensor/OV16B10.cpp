...
OV16B10::OV16B10(const T_SensorSetting &sensorSetting) :
    ImageSensor(sensorSetting)
{
    qDebug()<<__FUNCTION__;
}

int OV16B10::GetTemperature(int &temperature)
{
    QString strLog = "";
    USHORT temp = 0x00;
    ...
    {
        strLog = QString::asprintf("OV16B10::%s---I2cWrite Error---333\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        strLog = QString::asprintf("OV16B10::%s---I2cRead Error---444\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
     ...
     * ----20180404
     */
    strLog = QString::asprintf("OV16B10::%s---Temperature RegVal:0x%X\r\n", __FUNCTION__, temp);
    m_channelController->LogToWindow(strLog, qRgb(0, 0, 255));
    temperature = temp;
    if (temp > 0xC0)
    {
        /*strLog = QString::asprintf("OV16B10::%s---Attention, temp:0x%X is negative, so fixed 0°\r\n", __FUNCTION__, temp);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));*/
    ...
    if (ERR_NoError != I2cWrite(TEMPERATURE_CTL_REG_ADDR, temp & 0xFE, I2C_MODE_ADDR16_DATA8)) //Clear Bit[0]
    {
        strLog = QString::asprintf("OV16B10::%s---I2cWrite Error---555\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
 ...
 int OV16B10::GetSensorFuseId(std::string &fuseId, bool bUseMasterI2C)
 {
     (void)bUseMasterI2C;
     QString strLog = "";
     ...
     {
         strLog = QString::asprintf("OV16B10::%s---ReadReg Error\r\n", __FUNCTION__);
         m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
         return ERR_Failed;
     }
...
int OV16B10::GetSensorExposure(uint &value)
{
    QString strLog = "";
#if 1
    uchar Buf[] = {0, 0};
    ...
    {
        strLog = QString::asprintf("OV16B10::%s---ReadContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV16B10::SetSensorExposure(uint value)
{
    QString strLog = "";
#if 1
    ...
    {
        strLog = QString::asprintf("OV16B10::%s---WriteContinuousI2c() Error\r\n", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV16B10::GetSensorGain(uint &value)
{
    QString strLog = "";
#if 1
    uchar Buf[] = {0, 0};
...
int OV16B10::SetSensorGain(uint value, bool bMultiple)
{
    (void)bMultiple;
    QString strLog = "";
#if 1
    /** OV16B10和OV13A10的Gain设置方式一样，故参考《13A10_Gain_LookupTable.xlsx》里面Real Gain那两列里面的计算公式 **/
    if (true == bMultiple)
    {
        value = value * 256;
...
int OV16B10::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf, USHORT page)
{
    ...
    if (nullptr == buf)
    {
        strLog = QString::asprintf("OV16B10::%s---Error---buf is null", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
...
int OV16B10::ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf, USHORT page)
{
    ...
    {
        qDebug() << __FUNCTION__ << "  Error---startAddr larger than endAddr---startAddr:" << startAddr << " endAddr:" << endAddr;
        strLog = QString::asprintf("OV16B10::%s---Error---startAddr larger than endAddr---startAddr:0x%X endAddr:0x%X", __FUNCTION__, startAddr, endAddr);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        qDebug() << __FUNCTION__ << "  Error---new ptmpBuf Error---" << startAddr << " endAddr:" << endAddr << " nLen:" << nLen;
        strLog = QString::asprintf("OV16B10::%s---Error---new ptmpBuf Error---startAddr:0x%X endAddr:0x%X nLen:%d", __FUNCTION__, startAddr, endAddr, nLen);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        return ERR_Failed;
    }
    ...
    {
        qDebug() << __FUNCTION__ << "  WriteContinuousI2c Error";
        strLog = QString::asprintf("OV16B10::%s---WriteContinuousI2c Error---i2cParam.Addr:0x%X i2cParam.Speed:%d", __FUNCTION__, i2cParam.Addr, i2cParam.Speed);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return ec;
    }*/
...
//        {OTP_LOAD_CTRL,             OTP_LOAD_EN_WO}
    ...
    if (ERR_NoError != iRet)
    {
        ...
        strLog = QString::asprintf("OV16B10::%s---ReadContinuousI2c Error", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return ERR_Failed;
    }
    ...
    if (ERR_NoError != iRet)
    {
        ...
        strLog = QString::asprintf("OV16B10::%s---222---WriteContinuousI2c Error", __FUNCTION__);
        m_channelController->LogToWindow(strLog, qRgb(255, 0, 0));
        delete []ptmpBuf;
        return ERR_Failed;
    }*/
